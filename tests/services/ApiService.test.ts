...
describe('ApiService', () => {
  ...
  describe('isSupported', () => {
    ...
    it('support Brad.crypto', async () => {
      expect(await apiService.isSupported('Brad.crypto')).toEqual({ is_supported: true })
    })

    it('Brad.crypto is registered', async () => {
      expect(await apiService.isRegistered('Brad.crypto')).toEqual({ is_registered: true })
    })

    it('scorpiocat51.crypto is available', async () => {
      expect(await apiService.isAvailable('scorpiocat51.crypto')).toEqual({ is_available: true })
    })

    it('Brad.crypto\'s owner is 0x8aaD44321A86b170879d7A244c1e8d360c99DdA8', async () => {
      expect(await apiService.owner('Brad.crypto')).toEqual({ owner: '0x8aaD44321A86b170879d7A244c1e8d360c99DdA8' })
    })

    it('Brad.crypto\'s tokenId is 0x756e4e998dbffd803c21d23b06cd855cdc7a4b57706c95964a37e24b47c10fc9', async () => {
      expect(await apiService.tokenId('Brad.crypto')).toEqual({ token_id: '0x756e4e998dbffd803c21d23b06cd855cdc7a4b57706c95964a37e24b47c10fc9' })
    })

    it('Brad.crypto\'s address record is valid',
      async () => {
        expect(await apiService.record('Brad.crypto', 'address.eth')).toEqual(
          {
            key: 'address.eth',
            label: '',
            subtype: 'eth',
    ...
    it('Brad.crypto\'s records is valid',
      async () => {
        expect(await apiService.records('Brad.crypto')).toEqual(
          [
            {
              key: 'address.btc',
              type: 'address',
    ...
    it('Brad.crypto\'s addrs is valid',
      async () => {
        expect(await apiService.addrs('Brad.crypto', ['eth', 'btc'])).toEqual(
          [
            {
              key: 'address.eth',
              label: '',
    ...
    it('Brad.crypto\'s addr is valid',
      async () => {
        expect(await apiService.addr('Brad.crypto', 'eth')).toEqual(
          {
            key: 'address.eth',
            label: '',
            subtype: 'eth',
    ...
    it('Brad.crypto\'s registryAddress is valid',
      async () => {
        expect(await apiService.registryAddress('brad.crypto')).toEqual({ registry_address: '0xD1E5b0FF1287aA9f9A268759062E4Ab08b9Dacbe' })
      })
  })
})
