...
import {
  ...
  UnsupportedNameError,
} from '../errors/AllDIDError'
...
function findNamingServiceName (domain: string): NamingServiceName {
...
export function makeRecordItem (key: string): RecordItem {
...
function allDIDKeyToUnsKey (key: string): string {
  ...
  let unsKey = ''
  switch (keys[0]) {
    case KeyPrefix.Address:
      unsKey = `crypto.${keys[1].toUpperCase()}.address`
      break
    case KeyPrefix.Dweb:
      unsKey = `dweb.${keys[1].toLowerCase()}.hash`
      break
...
function unsKeyToAllDIDKey (key: string): string {
  ...
  switch (keys[0]) {
    case 'crypto':
      unsKey = `${KeyPrefix.Address}.${keys[1].toLowerCase()}`
      break

    case 'ipfs':
    case 'dweb':
      unsKey = `${KeyPrefix.Dweb}.${keys[1].toLowerCase()}_${keys[2]}`
      break
    ...
    case 'social':
    case 'forwarding':
      unsKey = `${KeyPrefix.Profile}.${keys[1].toLowerCase()}`
      break
  }
  return unsKey
}
...
export class UnsService extends NamingService {
  ...
  async records (name: string, keys?: string[]): Promise<RecordItem[]> {
    let recordObj
    if (keys) {
      recordObj = await this.uns.records(
        name,
        keys.map((key) => allDIDKeyToUnsKey(key))
      )
    }
    else {
    ...
    return recordKeys
      .filter((recordKey) => recordObj[recordKey])
      .map((recordKey) => {
  ...
  async addrs (
    ...
    if (!keys) {
      // TODO: there is a error,can't pass jest test
      const records = await this.uns.allNonEmptyRecords(name)
      const recordKeys = Object.keys(records).filter(
        (key) => key.split('.')[0] === 'crypto'
      )
      ...
      const records = await this.records(
        name,
        keys.map((key) => `${KeyPrefix.Address}.${key.toLowerCase()}`)
      )
  ...
  async addr (name: string, key: string): Promise<RecordItemAddr | null> {
    const recordItem = makeRecordItem(
      `${KeyPrefix.Address}.${key.toLowerCase()}`
    )
